...
async function loadAsPopupWithID(pageUrl, sessID) {
  ...
  const pageHTMLContent = await page.text()
  setHTMLasPopup(pageHTMLContent)
  if (pageUrl == "presentation-form.html") {
  ...
  setHTMLasPopup(`
    <form action="#" id="update-presentation-form" class="center">
            ...
            <div>
                ...
                <input type="time" name="startTime" id="startTime"  required value="${press.startTime}">
            </div>
            <div>
                <label for="endTime">Presentation Ends: </label>
                <input type="time" id="endTime" name="endTime"  required value="${press.endTime}">
            </div>
